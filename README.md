...
- 欢迎与授权引导：首次缺少输入监控权限时，会显示原生欢迎窗口，引导打开系统设置并试用快捷键拖拽。
- 监听自动恢复：全局输入监听被 macOS 临时禁用时，会先后台自动恢复；连续失败后才提示处理。
- 权限恢复提示：输入监控权限缺失或签名变化导致授权失效时，设置窗口会显示恢复说明。
...
在菜单栏中点击 DragFrame 图标，可打开设置、重新查看欢迎引导、暂停/启用应用、修改快捷键、调整边框颜色或开启登录时自动启动。
